...
def get_reading_list(user):
    reading_query = db.session.query(BookList.dateAdded, Book.id, Book.googleId, Book.title, Book.subtitle, Book.author, Book.publishDate, Book.image, Book.small_image, BookList.priority, User.username).outerjoin(User, BookList.recommendedBy == User.id).join(Book, BookList.bookId == Book.id).filter(BookList.userId == user.id).all()
    reading_list = []
    print("finding Reading List")
    for books in reading_query:
